...
public final class TableReaderWriterTest {
	...
	public void testWritePostingList() throws Exception {
		...
		TableWriter<PostingList> writer = new TableWriter<>(tmpFile, false, 1);
		PostingList list = new PostingList();
		list.addItem(new PostingListItem(5, 42));
		writer.put("k1", list);
		writer.close();

		TableReader<PostingList> reader = new TableReader<>(tmpFile, PostingList.class);
